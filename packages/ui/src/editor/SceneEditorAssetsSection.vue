...
function placeAsset(assetId: string): void {
...
// shallowRef: Map mutations are not deep-tracked by Vue; reassigning .value triggers reactivity.
...
watch(
  ...
  (current) => {
    ...
    for (const a of current) {
      if (a.thumbnail && !next.has(a.id)) {
        next.set(a.id, URL.createObjectURL(a.thumbnail))
      }
    }
...
.asset-detail-trigger:hover {
  background: rgba(90, 176, 245, 0.07);
}
